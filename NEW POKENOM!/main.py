...
async def info(ctx):
    author = ctx.author.name  # Dapatkan nama pengguna yang menggunakan perintah
    if author in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
...
async def feed(ctx):
    ...
    if author in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
        pokemon = Pokemon.pokemons[author]
        await ctx.send(await pokemon.feed())  # Mengirim informasi tentang Pokémon 
    else:
        await ctx.send("Kamu belum berhasil mendapatkan Pokémon!")  # Menginformasikan kepada pengguna bahwa Pokémon telah dibuat
